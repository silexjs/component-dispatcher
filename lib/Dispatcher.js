var Dispatcher = function(options) {
	...
	this.console = options.console || function(m) {
...
Dispatcher.prototype = {
	...
	set: function(listener, callback, priority, serviceName) {
		...
		if(this.debug.set === true) {
			this.console('set "'+listener+'" ('+(-priority)+(serviceName!==null?' | '+serviceName:'')+')');
		}
	...
	dispatch: function(listenerName, args, callbackFinal, callbackException) {
		...
		if(debug === true) {
			this.console('dispatch "'+listenerName+'" (start)');
		}
		...
		var next = function(e) {
			if(e !== undefined && e instanceof Error) {
				if(debug === true) {
					self.console('dispatch "'+listenerName+'" (error)');
				}
			...
			try {
				if(nextI < nextListLength) {
					...
					if(debug === true) {
						self.console('dispatch "'+listenerName+'" (end)');
					}
